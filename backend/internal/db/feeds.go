...
func (d *DB) ListFeeds(ctx context.Context, categoryID *int64) ([]Feed, error) {
	...
	if categoryID != nil {
		rows, err = d.Pool.Query(ctx,
			`SELECT id, category_id, name, url, etag, last_modified, last_fetched_at, error_count, is_active, created_at
			 FROM feeds WHERE category_id = $1 ORDER BY name ASC`, *categoryID)
	} else {
		rows, err = d.Pool.Query(ctx,
			`SELECT id, category_id, name, url, etag, last_modified, last_fetched_at, error_count, is_active, created_at
	...
	for rows.Next() {
		var f Feed
		if err := rows.Scan(&f.ID, &f.CategoryID, &f.Name, &f.URL, &f.ETag, &f.LastModified,
			&f.LastFetchedAt, &f.ErrorCount, &f.IsActive, &f.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
...
func (d *DB) GetFeed(ctx context.Context, id int64) (*Feed, error) {
	...
	err := d.Pool.QueryRow(ctx,
		...
		Scan(&f.ID, &f.CategoryID, &f.Name, &f.URL, &f.ETag, &f.LastModified,
...
func (d *DB) CreateFeed(ctx context.Context, categoryID int64, name, url string) (*Feed, error) {
	...
	var f Feed
	err := d.Pool.QueryRow(ctx,
		`INSERT INTO feeds (category_id, name, url) VALUES ($1, $2, $3)
		 RETURNING id, category_id, name, url, etag, last_modified, last_fetched_at, error_count, is_active, created_at`,
		categoryID, name, url).
		Scan(&f.ID, &f.CategoryID, &f.Name, &f.URL, &f.ETag, &f.LastModified,
			&f.LastFetchedAt, &f.ErrorCount, &f.IsActive, &f.CreatedAt)
	if err != nil {
...
func (d *DB) UpdateFeed(ctx context.Context, id int64, categoryID *int64, name, url *string, isActive *bool) (*Feed, error) {
	...
	q := "UPDATE feeds SET " + strings.Join(sets, ", ") + " WHERE id = $" + itoa(i) +
		" RETURNING id, category_id, name, url, etag, last_modified, last_fetched_at, error_count, is_active, created_at"
	var f Feed
	err := d.Pool.QueryRow(ctx, q, args...).
		Scan(&f.ID, &f.CategoryID, &f.Name, &f.URL, &f.ETag, &f.LastModified,
			&f.LastFetchedAt, &f.ErrorCount, &f.IsActive, &f.CreatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
